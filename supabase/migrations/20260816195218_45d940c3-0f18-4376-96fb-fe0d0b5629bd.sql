-- ============================================================================
-- Global, race-safe uniqueness for public handles and chosen display names.
-- Additive only: no user data is deleted or renamed automatically.
-- ============================================================================
...
/* ------------------- 1. fail-closed pre-flight duplicate check ------------- */

DO $preflight$
DECLARE
  v_dupes text;
BEGIN
  -- current handles colliding with each other after normalisation
  ...
  -- redirect handles colliding with a foreign current handle
  ...
  -- redirect handles colliding with each other
  SELECT string_agg(k, ', ') INTO v_dupes FROM (
  ...
  -- chosen display names colliding after NFKC/case/trim normalisation
  SELECT string_agg(k, ', ') INTO v_dupes FROM (
...
-- Internal registry: never reachable through the public Data API.
...
DECLARE
  ...
  v_clean text;
BEGIN
  -- `anonymous_identities.handle` is deprecated as an independent namespace:
  -- it may only mirror the authoritative `user_rooms.handle`.
  NEW.handle := (
...
/* -------------------- 6. deterministic free-name search ------------------- */
...
/* ------------------- 7. atomic personal-room provisioning ----------------- */
...
BEGIN
  ...
  -- Win the claim first: a foreign current or redirect handle blocks the change.
  IF NOT public.try_claim_name('handle', v_norm, p_subject_hash) THEN
    RAISE EXCEPTION 'ALIAS_TAKEN' USING ERRCODE = '23505';
  END IF;

  -- Reclaiming one's own old handle: drop that redirect before it becomes current.
...
BEGIN
  ...
  -- The handle stays untouched on purpose: only change_personal_handle moves it.
